...
Windows: Install WinRAR or download unrar.dll from unrarlib.
...
Freqtrade (Optional): For integration with Freqtrade, follow the Freqtrade installation guide.
...
Clone the repository:git clone https://github.com/vuongductuanktmt/crypto-trading-scripts.git
cd crypto-trading-scripts


Install dependencies:pip install --upgrade pandas inquirer requests


Save the script (get_top_200_futures_by_exchange.py) in the repository folder.

Usage

Run the script:python get_top_200_futures_by_exchange.py


Select an exchange from the interactive menu (e.g., kucoin):? Select an exchange: kucoin
...
Check API data:import requests
...
Add the following line before if ccxt_symbol: in the get_futures_tickers function:print(f"Raw Symbol: {symbol}, CCXT Symbol: {ccxt_symbol}, Base Token: {base_token if ccxt_symbol else 'None'}")
...
Install dependencies:pip install --upgrade pandas python-binance numpy


Save the script (candle_signal_analysis.py) in the repository folder.

Usage

Run the script:python candle_signal_analysis.py
...
Check API:from binance.client import Client
client = Client()
...
Install dependencies:pip install --upgrade rarfile


Install system-level unrar dependency:
Windows: Install WinRAR or download unrar.dll from rarlab.com and place it in the script directory or system path.
Linux: Install unrar via sudo apt-get install unrar.
macOS: Install unrar via Homebrew (brew install rar).


Save the script (brute_force_rar.py) in the repository folder.
Prepare a RAR file (e.g., profile_19112017.rar) and place it in the script directory.

Usage

Modify the script to specify:
rar_file: Path to the RAR file (e.g., ./profile_19112017.rar).
...
Run the script:python brute_force_rar.py
...
Check RAR File:
Ensure the file exists and is not corrupted:import rarfile
try:
    with rarfile.RarFile('./[filename].rar') as rar:
...
Add or update the script and document it in this README.md.
Commit changes (git commit -m "Add new script XYZ").
Push to your fork (git push origin feature/new-script).
Open a pull request.
